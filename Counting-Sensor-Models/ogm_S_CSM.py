...
class ogm_S_CSM:
    ...
    def construct_map(self, pose, scan):
        ...
        # -----------------------------------------------
        self.map['mean'] = 0.5*np.ones((t.shape[0], (self.num_classes+1)))        # size should be (number of data) x (number of classes + 1)
        self.map['variance'] = 0.5*np.ones((t.shape[0],1))    # size should be (number of data) x (1)
        self.map['alpha'] = 0.5*np.ones((t.shape[0], (self.num_classes+1)))       # size should be (number of data) x (number of classes + 1)
    ...
    def build_ogm(self):
        ...
        for i in tqdm(range(self.map['size'])):
            ...
            alpha_star = max(self.map['alpha'][i, :])
